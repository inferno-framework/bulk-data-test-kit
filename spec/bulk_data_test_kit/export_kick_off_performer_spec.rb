...
class ExportKickOffPerformerTesterClass < Inferno::Test
  ...
  attr_accessor :bulk_auth_info, :group_id, :requests
...
RSpec.describe BulkDataTestKit::ExportKickOffPerformer do
  ...
  before do
    performer.bulk_auth_info = Inferno::DSL::AuthInfo.new({auth_type: :backend_services, access_token: token})
  ...
  describe 'perform_export_kick_off_request' do
    ...
    it 'raises skip if use_token but token is not present' do
      performer.bulk_auth_info = Inferno::DSL::AuthInfo.new({ auth_type: :backend_services })
