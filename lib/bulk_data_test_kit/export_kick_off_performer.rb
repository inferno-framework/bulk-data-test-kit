...
module BulkDataTestKit
  module ExportKickOffPerformer
    def perform_export_kick_off_request(use_token: true, params: {}, url: '')
      skip_if use_token && bulk_auth_info.access_token.blank?, 'Could not verify this functionality when bearer token is not set'

      headers = { accept: 'application/fhir+json', prefer: 'respond-async' }
      headers.merge!({ authorization: "Bearer #{bulk_auth_info.access_token}" }) if use_token
    ...
    def delete_export_kick_off_request
      ...
      headers = { accept: 'application/json', authorization: "Bearer #{bulk_auth_info.access_token}" }
