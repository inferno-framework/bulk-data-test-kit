...
module BulkDataTestKit
  module BulkExportValidationTester
    ...
    def build_headers(use_token)
      ...
      headers.merge!({ authorization: "Bearer #{bulk_auth_info.access_token}" }) if use_token == 'true'
      headers
    end
    ...
    def perform_bulk_export_validation(bulk_status_output: '', bulk_requires_access_token: '')
      ...
      skip_if (bulk_requires_access_token == 'true' && bulk_auth_info.access_token.blank?),
    ...
    def ndjson_download_requiresAccessToken_check(bulk_data_download_url: '', bulk_requires_access_token: '')
      ...
      skip_if bulk_auth_info.access_token.blank?, 'Could not verify this functionality when Bearer Token is not provided'
